...
/**
 * 1. Fonction pour obtenir le token d'accès OAuth2 d'Orange.
 */
function getAccessToken() {
    $url = "https://api.orange.com/oauth/v3/token";
    
    // Remplacez par vos identifiants Orange (Client ID et Client Secret)
    $client_id = "kwKdtG7tzc26fW37QTg3qxivib3mDv71";
    $client_secret = "8UEqTlKIXRLnJru6NMHa8uBmouQU9x9x1A4KzCounl2W";
    
    // Calculer la chaîne d'authentification (Base64(client_id:client_secret))
    ...
    if (isset($decoded['access_token'])) {
        ...
        error_log("Erreur lors de la récupération du token d'accès. Réponse: $response");
        return false;
    }
}
...
function sendSmsNotification($to, $message) {
    ...
    // Préparez le corps de la requête au format requis par Orange
    $body = array(
        "outboundSMSMessageRequest" => array(
            // Adresse du destinataire au format "tel:+221787368793"
            "address" => "tel:" . $to,
    ...
    if ($status_code != 201) {
        error_log("Échec de l'envoi du SMS. Code HTTP: $status_code. Réponse: $response");
        return false;
    }
...
if (isset($_POST['submit'])) {
    ...
    $query .= "VALUES('$billingnum', '$custname', '$custmobilenum', '$modepayment', '$netTotal');";
    ...
    if ($result) {
        ...
        // Préparer le SMS
        // Le numéro du client doit être au format international, par exemple "+221787368793"
